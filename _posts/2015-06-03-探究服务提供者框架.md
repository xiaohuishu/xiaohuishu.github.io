...
> 2.之后分析1.2操作:Class.forName("com.mysql.jdbc.Driver");实际功能是加载mysql数据库驱动,Class.forName(className)是以全限定的类名获得这个类的Class对象,所以我们可以知道获取到了com.mysql.jdbc.Driver类的Class对象,但是只是这样貌似并没有完成加载驱动这个功能,不能忽略一点：Class.forName(className)方法会执行加载类的静态块(static)块,我们可以在官网下载mysql-connect-java-x.x.x.zip源码包.打开Driver类源码可以看到：
