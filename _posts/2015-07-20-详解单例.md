...
> * 饱汉,就是实例对象一开始就已创建好(这里的开始指的是类初始化阶段);
...
这样一个饱汉式单例就完成了,但是这个类存在两个隐患和一个优势：
...
这样一个饿汉式单例产生了,但是这个类有3个隐患:

隐患:

> * 1.线程不安全(PS:画的图没保存,下次补上);

线程A执行if(single == null){ ... }的时候线程B此时也执行到if(single == null),而A并没有实例化所以B正好完成实例化操作;

但A此时没有响应到B中对single的操作,所以A也进行实例化操作;
